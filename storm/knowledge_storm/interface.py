            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:  # Don't sleep on the last attempt
                        logging.warning(f"Attempt {attempt + 1} failed for query '{q}': {str(e)}. Retrying in {retry_delay} seconds...")
                        time.sleep(retry_delay)
                        retry_delay *= 2  # Exponential backoff
                    else:
                        logging.error(f"All {max_retries} attempts failed for query '{q}': {str(last_error)}")
